...
final class Phase2InstallerTests: XCTestCase {
    var dir: URL!
    let bridge = URL(fileURLWithPath: "/Applications/MegaMicro.app/Contents/Resources/MegaMicroBridge")
    override func setUpWithError() throws {
        dir = FileManager.default.temporaryDirectory.appendingPathComponent("megamicro-phase2-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    }
    override func tearDown() { try? FileManager.default.removeItem(at: dir) }

    func testOpenCodePluginInstallAndUninstall() throws {
        let url = dir.appendingPathComponent("plugins/megamicro.ts")
        let installer = OpenCodeIntegration(pluginURL: url, port: 48802)
        try installer.install()
        let text = try String(contentsOf: url)
        XCTAssertTrue(text.contains("permission.asked"))
        XCTAssertTrue(text.contains("127.0.0.1:48802/state"))
        try installer.uninstall()
        XCTAssertFalse(installer.isInstalled())
    }

    func testCopilotWritesOwnedNativeHookFile() throws {
        let url = dir.appendingPathComponent("hooks/megamicro.json")
        let installer = CopilotIntegration(hooksURL: url, port: 48802, bridgeURL: bridge)
        try installer.install()
