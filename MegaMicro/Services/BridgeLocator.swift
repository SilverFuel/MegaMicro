...
enum BridgeLocator {
    static var bundledURL: URL {
        if let url = Bundle.main.resourceURL?.appendingPathComponent("MegaMicroBridge"),
        ...
        return Bundle.main.bundleURL.deletingLastPathComponent().appendingPathComponent("MegaMicroBridge")
    }

    static var installedURL: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("MegaMicro/bin/MegaMicroBridge")
    }
    ...
    static func command(provider: String, event: String, port: UInt16, url: URL? = nil) -> String {
        ...
        return "'\(path)' --provider \(provider) --event \(event) --port \(port) #megamicro"
    }
}
