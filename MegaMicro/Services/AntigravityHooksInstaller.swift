...
/// conversation/workspace and forwards a normal MegaMicro state report.
final class AntigravityHooksInstaller {
    static let hookName = "megamicro"
    enum InstallError: LocalizedError {
        case malformedHooks
        var errorDescription: String? { "Antigravity hooks.json is malformed; MegaMicro left it untouched." }
    }
    ...
    func install() throws {
        ...
        let handler: [String: Any] = [
            "type": "command",
            "command": "\(shellQuote(bridgeURL.path)) --provider antigravity --event %EVENT% --port \(port) #megamicro",
            "timeout": 5,
        ]
